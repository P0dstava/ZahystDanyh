...
public:
    ...
    void update(const unsigned char* input, size_t length) {
        ...
        if ((count[0] += ((uint32)length << 3)) < ((uint32)length << 3))
            count[1]++;
    ...
    void update(const std::string &str) {
        update((const unsigned char*)str.c_str(), str.size());
    }
...
private:
    ...
    static void encode(unsigned char* output, uint32* input, size_t len) {
...
#define II(a, b, c, d, x, s, ac) { \
    ...
    static void decode(uint32* output, const unsigned char* input, size_t len) {
        for (size_t i = 0, j = 0; j < len; i++, j += 4)
